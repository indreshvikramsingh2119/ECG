...
def bandpass_filter(data, lowcut=0.5, highcut=40, fs=50, order=4):
    ...
    if low <= 0 or high >= 1 or low >= high:
        return data  # Return unfiltered data if parameters are invalid
    b, a = butter(order, [low, high], btype='band')
    # Ensure enough data for filtfilt
...
def update(frame):
    try:
        ...
        if line_data.isdigit():
            ...
            # Only filter if enough data is present
            if len(data_buffer) > 30:
                filtered_data = bandpass_filter(data_buffer)
                line.set_ydata(filtered_data)
                bpm = calculate_bpm_and_label_waves(filtered_data)
            else:
